...
export default function ModalRecarregar({minutosRestantes, atualizarCronFn}: Props){
    ...
    function confereSaldo(){
        fetch("http://localhost:8080/pegarPrevSaldo", {headers: {"authorization": localStorage.getItem("authToken")? `Bearer ${localStorage.getItem("authToken")}` : "", "Content-Type": "application/json"}}).then(res => res.json()).then(data => {
            if(data[0] == "sucesso"){
                if(data[1]){
                    if(data[1].previsaoSaldo){
                        if(data[1].previsaoSaldo >= precoAdicionalConsulta){
                            ...
                            fetch("http://localhost:8080/atualizarTempoConsulta", {
                                method: "POST",
