...
export default function Chat({atendente, minutosAtendenteFn, segundosAtendenteFn, }: Props){
  ...
  function enviarFn(){
    ...
    /*async function pegarInfoCliente(id: string): Promise<{ nome: string; email: string; }> {
      fetch("https://167.88.32.149:8080/pegarInfoCliente", {
        method: "POST",
...
*/
    ...
    useEffect(() => {
      ...
      socket.on("salaEncerrada", () => {
        if(atendente && infoSalas){
         /* let infoSalasClone = [...infoSalas]
          ...
          setInfoSalas(infoSalasClone)*/
          fetch("https://167.88.32.149:8080/buscarSalasAtendente", {
            headers: {"authorization": localStorage.getItem("authToken")? `Bearer ${localStorage.getItem("authToken")}` : ""}
          }).then(res => res.json()).then(data => {
            console.log(data)
            if(data[0] !== "erro"){
      ...
      socket.on("clienteChamando", (data) => {
        ...
        if(atendente){
          ...
          fetch("https://167.88.32.149:8080/infoAtendente", {
            headers: {"authorization": localStorage.getItem("authToken")? `Bearer ${localStorage.getItem("authToken")}` : ""}
    ...
    useEffect(() => { 
      ...
      if(atendente){
        ...
        fetch("https://167.88.32.149:8080/minhasInfosAtendente", {
          headers: {"authorization": localStorage.getItem("authToken")? `Bearer ${localStorage.getItem("authToken")}` : ""}
        }).then(res => res.json()).then(data => {
          if(data[0] == "sucesso"){
        ...
        fetch("https://167.88.32.149:8080/confereTokenUsuario", {headers: {"authorization": localStorage.getItem("authToken")? `Bearer ${localStorage.getItem("authToken")}` : ""}}).then(res => res.json()).then(data => {
          ...
          fetch("https://167.88.32.149:8080/buscarSalaUsuario", {headers: {"authorization": localStorage.getItem("authToken")? `Bearer ${localStorage.getItem("authToken")}` : ""}}).then(res => res.json()).then(data => {
            if(data[0] == "erro"){
              setTemAviso(true)/*  */
              if(data[1]){
                setTextoAviso(data[1])
        ...
        fetch("https://167.88.32.149:8080/infoMeuAtendente", {headers: {"authorization": localStorage.getItem("authToken")? `Bearer ${localStorage.getItem("authToken")}` : ""}}).then(res => res.json()).then(data => {
          if(data[0] == "sucesso"){
            setPerfilProAtual(data[1])
            console.log(data[1])
          }else{
...
*/
    ...
    function atualizarCronometro(){
      fetch("https://167.88.32.149:8080/quantoFalta" + (atendente? "Atendente": "Cliente"), {headers: {"authorization": localStorage.getItem("authToken")? `Bearer ${localStorage.getItem("authToken")}` : ""}}).then(res => res.json()).then(data => {
        if(data[0] == "sucesso"){
                ...
                <div className=' bg-white/30 backdrop-blur-md p-4 grid grid-cols-3 gap-4 items-center justify-items-center'>
                  {
                    ...
                    baralhosUrls.map((item, index) => {
                      return <Carta urlImg={`https://167.88.32.149:8080/images/${item}`} cartasSelecionadas={cartasSelecionadas} setCartasSelecionadas={setCartasSelecionadas} idxCarta={index} />
                    })
                  }
                </div>
                {
